    ...
    private static void RunPatch(IPatcherState<ISkyrimMod, ISkyrimModGetter> state)
    {
        ...
        if (waterMods.Count > 0)
        {
            ...
            foreach (var (formKey, source) in cells)
            {
                ...
                var currentMap = patchCells.TryGetValue(formKey, out var patchedCell)
                    ? patchedCell.WaterEnvironmentMap
                    : winner.Record.WaterEnvironmentMap;

                var currentHeight = patchCells.TryGetValue(formKey, out patchedCell)
                    ? patchedCell.WaterHeight
                    : winner.Record.WaterHeight;

                if (currentMap == source.WaterEnvironmentMap && currentHeight == source.WaterHeight)
                    continue;
                ...
                cell.WaterHeight = source.WaterHeight;
                patchCells[formKey] = cell;
                patched++;
            }

            Console.WriteLine($"Patched {patched} CELL water settings");
        }
    }
}
